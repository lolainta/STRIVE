...
def add_base_args(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="Number of future (output prediction) timesteps to use.",
    )
    parser.add_argument(
        "--agent_types",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="width of the map observation around each agent (in pixels)",
    )
    parser.add_argument(
        "--map_obs_bounds",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["drivable_area", "carpark_area", "road_divider", "lane_divider"],
        help="which nuscenes map layers to return from the environment.",
    )

    # model options
    parser.add_argument(
        "--ckpt", type=str, default=None, help="Path to model weights to load in."
    )
    parser.add_argument(
        "--map_feat_size",
        type=int,
        default=64,
        help="Feature size for map crop encoding.",
    )
    parser.add_argument(
        "--past_feat_size",
        type=int,
        default=64,
        help="Feature size for past trajectory encoding.",
    )
    parser.add_argument(
        "--future_feat_size",
        type=int,
        default=64,
        help="Feature size for future traj encoding.",
    )
    parser.add_argument(
        "--latent_size", type=int, default=32, help="CVAE latent space dim."
    )
    parser.add_argument(
        "--no_output_bicycle",
        dest="model_output_bicycle",
        action="store_false",

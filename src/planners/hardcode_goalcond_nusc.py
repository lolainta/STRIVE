...
CONFIG_DICT = {"default": DEF_CONFIG, "final_tuned_val_1": TUNED_VAL_FINAL_1}
...
class HardcodeNuscPlanner(PlannerNusc):
    ...
    def create_init_state(self, init_state, vehicle_atts, B, batch_mask):
        ...
        for b in range(B):
            ...
            cur_init_state = self.state_conv(
                init_state[cur_mask], vehicle_atts[cur_mask]
            )
            init_state_list.append(cur_init_state)
        return init_state_list

    def reset(
        self, init_state, vehicle_atts, batch_mask, batch_size, map_idx, ego_idx=0
    ):
        """
        ...
        self.init_wstate = self.create_init_state(
            init_state, vehicle_atts, self.B, self.batch_mask
        )
    ...
    def create_other_agents(self, init_agts, agt_obs, agt_t):
        ...
        for aidx in range(agt_obs.shape[0]):
            ...
            first_nan_idx = (
                all_states.shape[0] if first_nan_idx.shape[0] == 0 else first_nan_idx[0]
            )
            if first_nan_idx == 1:
                # only a single non-nan value, skip it
                cur_traj = [
                    {"t": 0.0}
                ]  # must include a traj though to be valid in rollout
    ...
    def rollout(
        ...
        """
        if (
            (init_state is None and self.init_wstate is None)
            or self.B is None
        ...
        if init_state is not None:
            batch_wstate = self.create_init_state(
                init_state, vehicle_atts, self.B, self.batch_mask
            )
        ...
        if agent_obs is not None:
            ...
            for b in range(self.B):
                ...
                cur_agt_data = self.create_other_agents(
                    batch_wstate[b], cur_agts, agent_t
                )
        ...
        for b in range(self.B):
            ...
            compute_action(
                ...
                cfg.col_plim,
                debug=False,
                score_wmin=cfg.score_wmin,
                score_wfac=cfg.score_wfac,
            )
            ...
            if viz is not None:
                viz_wstate(
                    ...
                    lane_graph,
                    70.0,
                    os.path.join(viz, "%04d.jpg" % (b * (Tsteps + 1))),
                )
            ...
            for istep in range(Tsteps):
                ...
                if viz is not None:
                    viz_wstate(
                        ...
                        lane_graph,
                        70.0,
                        os.path.join(viz, "%04d.jpg" % (b * (Tsteps + 1) + 1 + istep)),
                    )
        ...
        if viz is not None:
            create_video(
                os.path.join(viz, "%04d.jpg"), viz + ".mp4", 2 * int(1.0 / cfg.dt)
            )
            shutil.rmtree(viz)
...
#
def compute_splines(
    wstate, lane_graph, cdisttheta, xydistmax, lane_ds, lane_sig, sbuffer, smax, tmax
):
    assert smax > 0, f"{smax}"
    for objid, obj in wstate["objs"].items():
        matches = get_lane_matches(
            obj["x"],
            obj["y"],
            obj["h"],
            lane_graph,
        ...
        obj["final_matches"] = cluster_matches_combine(
            obj["x"], obj["y"], matches, lane_graph
        )
        backdist = 1.0 if obj["s"] > 0 else 1.0 + abs(obj["s"]) * tmax
        fordist = (
            1.0 + smax * tmax
            if obj["s"] < 0
            else max(1.0 + smax * tmax, 1.0 + obj["s"] * tmax)
        )
        obj["splines"] = get_prediction_splines(
            obj["final_matches"],
            lane_graph,
            backdist=backdist,
            fordist=fordist,
            xydistmax=xydistmax,
            egoxy=np.array([obj["x"], obj["y"]]),
            lane_ds=lane_ds,
            lane_sig=lane_sig,
            sbuffer=sbuffer,
            egoh=obj["h"],
...
def get_lane_matches(x, y, h, lane_graph, cdistmax, xydistmax):
    ...
    cdist = (
        1.0
        - lane_graph["edges"][:, 2] * np.cos(h)
        - lane_graph["edges"][:, 3] * np.sin(h)
    )
    kept = cdist < cdistmax
...
def cluster_matches_combine(x, y, matches, lane_graph):
    ...
    ordering = np.argsort(
        np.linalg.norm(np.array([[x, y]]) - matches["closest"], axis=1)
    )
...
def constant_heading_spline(egoxy, egoh, backdist, fordist):
    ...
    x = np.array(
        [
            [
                egoxy[0] - backdist * np.cos(egoh),
                egoxy[1] - backdist * np.sin(egoh),
                np.cos(egoh),
                np.sin(egoh),
            ],
            [
                egoxy[0] + fordist * np.cos(egoh),
                egoxy[1] + fordist * np.sin(egoh),
                np.cos(egoh),
                np.sin(egoh),
            ],
        ]
    ...
    return interp1d(
        t, x, kind="linear", axis=0, copy=False, bounds_error=True, assume_sorted=True
    )


def get_prediction_splines(
    final_matches,
    lane_graph,
    backdist,
    fordist,
    ...
    for (v0, v1), close in zip(final_matches["ixes"], final_matches["closest"]):
        forward_lanes = expand_verts(
            v1,
            lane_graph["xy"],
            lane_graph["out_edges"],
            mindist=fordist + sbuffer + xydistmax,
        )
        backward_lanes = expand_verts(
            v0,
            lane_graph["xy"],
            lane_graph["in_edges"],
        ...
        for flane in forward_lanes:
            for blane in backward_lanes:
                ...
                if flane["l"] <= fordist + sbuffer + xydistmax:
                    xys = extend_forward(
                        xys, 1.0 + fordist + sbuffer + xydistmax - flane["l"]
                    )
                if blane["l"] <= backdist + sbuffer + xydistmax:
                    xys = extend_backward(
                        xys, 1.0 + backdist + sbuffer + xydistmax - blane["l"]
                    )
                    ix0 += 1
                ...
                if tdist[0] >= -backdist - sbuffer:
                    print(
                        f"WARNING: spline not far enough back {tdist[0]} {-backdist-sbuffer}"
                    )
                if tdist[-1] <= fordist + sbuffer:
                    print(
                        f"WARNING: spline not far enough forward {tdist[-1]} {fordist+sbuffer}"
                    )
                interp = interp1d(
                    tdist,
                    xys,
                ...
                xys = (
                    xys
                    + (egoxy - cclose)[np.newaxis, :]
                    * np.exp(-np.square(teval) / lane_sig**2)[:, np.newaxis]
                )

                spline = xy2spline(
                    xys, numback, blim=-backdist, flim=fordist, egoh=egoh
                )
...
def viz_wstate(wstate, lane_graph, window, imname):
    ...
    mag = 0.3
    plt.plot(
        lane_graph["edges"][:, 0] + mag * lane_graph["edges"][:, 2],
        lane_graph["edges"][:, 1] + mag * lane_graph["edges"][:, 3],
        ".",
        markersize=1,
    )
    ...
    for objid, obj in wstate["objs"].items():
        plt.plot(
            obj["final_matches"]["closest"][:, 0],
            obj["final_matches"]["closest"][:, 1],
            "b.",
            markersize=8,
            alpha=0.3,
        )
...
def update_wstate(wstate, v, dt):
    ...
    for objid, obj in wstate["objs"].items():
        if "control" in obj:
            ...
            v["objs"][objid]["traj"][0]["t"] <= t1 <= v["objs"][objid]["traj"][-1]["t"]
        ):
...
def collect_other_trajs(
    wstate, egoid, nsteps, preddt, predsfacs, predafacs, interacdist, maxacc
):
    ...
    for otherid, other in wstate["objs"].items():
        if (
            otherid == egoid
            or np.sqrt((egox - other["x"]) ** 2 + (egoy - other["y"]) ** 2)
            > interacdist
        ):
            continue
        sprofs = [
            ...
            for sfac in predsfacs
            for afac in predafacs
        ]
...
def debug_planner(egotraj, otherobjs, lane_graph, debugname, prob, egoobj, sprofi):
    ...
    for ti in range(otherobjs.shape[0]):
        ...
        # plot lane graph
        plt.plot(
            lane_graph["edges"][:, 0], lane_graph["edges"][:, 1], ".", markersize=2
        )
        mag = 0.3
        plt.plot(
            lane_graph["edges"][:, 0] + mag * lane_graph["edges"][:, 2],
...
def plot_plan_info(
    ...
    for sprofi, sprof in enumerate(sprofs):
        ...
        if debug:
            debug_planner(
                egotraj, otherobjs, lane_graph, debugname, prob, egoobj, sprofi
            )
    ...
    for fac in planaccfacs:
        ...
        for s1 in np.linspace(sbot, stop, NS):
            ...
            for s2 in np.linspace(sbot, stop, NS):
                ...
                sprofs.append(
                    {"sprof": sprof, "teval": teval, "acc": acc, "s1": s1, "s2": s2}
                )
...
def compute_action(
    ...
    sprofs = gen_sprofiles(
        obj["s"], preddt, nsteps, planaccfacs, maxacc, smax, plannspeeds, debugname
    )
    otherobjs = collect_other_trajs(
        wstate, objid, nsteps, preddt, predsfacs, predafacs, interacdist, maxacc
    )
    sprof = plot_plan_info(
        otherobjs,
        sprofs,
        debugname,
        obj,
        ...
        debug=debug,
        score_wmin=score_wmin,
        score_wfac=score_wfac,
    )
    ...
    newx, newy, newh = postprocess_act_for_speed(
        obj["x"], obj["y"], obj["h"], newx, newy, newh, stgt, dt
    )
...
def approx_bbox_distance(b0, b1):
    ...
    dist = (
        np.linalg.norm(bc1[:, :, :, :, :, [0, 1]] - bc0[:, :, :, :, :, [0, 1]], axis=5)
        - bc0[:, :, :, :, :, 2]
        - bc1[:, :, :, :, :, 2]
    )
...
def plot_car(x, y, h, l, w, color="b", alpha=0.5, no_heading=False):
    plot_box(
        np.array([x, y, np.cos(h), np.sin(h)]),
        [l, w],
        color=color,
        alpha=alpha,

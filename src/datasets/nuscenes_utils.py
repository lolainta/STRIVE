...
def get_nusc_maps(data_folder):
    nusc_maps = {
        map_name: NuScenesMap(dataroot=data_folder, map_name=map_name)
        for map_name in [
...
def process_lanegraph(nmap, res_meters, eps):
    ...
    for lane in nmap.lane + nmap.lane_connector:
        ...
        check_duplicates(
            discretized, eps
        )  # make sure each point is at least eps away from neighboring points
    ...
    for lid, lane in lane_graph.items():
        ...
        for outtok in nmap.connectivity[lid]["outgoing"]:
            if outtok in lane_graph:
                out_edges[laneid2start[lid] + len(lane) - 1].append(
                    laneid2start[outtok]
                )
        for intok in nmap.connectivity[lid]["incoming"]:
            if intok in lane_graph:
                in_edges[laneid2start[lid]].append(
                    laneid2start[intok] + len(lane_graph[intok]) - 1
                )
...
def velocity(pos, t):
    ...
    vel = np.concatenate(
        [vel_diff[0:1, :], vel_diff], axis=0
    )  # for first frame use forward diff
...
def gen_car_coords(xys, hs, C, L, W, bounds=None, ls=None, ws=None):
    ...
    if bounds is not None:
        lwise = (
            torch.linspace(bounds[0], bounds[2], L, device=device)
            .view(1, 1, L, 1)
            .expand(B, C, L, W)
        )
        wwise = (
            torch.linspace(bounds[1], bounds[3], W, device=device)
            .view(1, 1, 1, W)
            .expand(B, C, L, W)
        )
    elif ls is not None and ws is not None:
        lwise = (
            torch.linspace(-1.0, 1.0, L, device=device)
            .view(1, 1, L, 1)
            .expand(B, C, L, W)
            * ls.view(B, 1, 1, 1)
            / 2
    ...
    xys = torch.stack(
        (lwise * hcos - wwise * hsin, lwise * hsin + wwise * hcos), 4
    ) + xys.view(B, 1, 1, 1, 2)
...
def check_on_layer(drivables, dx, cars, lw, mapixes):
    ...
    B = cars.size(0)
    # sample grid aligned with agent coords in the world space
    xys = gen_car_coords(cars[:, :2], cars[:, 2:], 1, L, W, ls=lw[:, 0], ws=lw[:, 1])[
        :, 0
    ]  # don't need multiple channels
    ...
    outside = (
        (xys[:, :, :, 1] < 0)
        | (xys[:, :, :, 1] >= drivables.shape[1])
        | (xys[:, :, :, 0] < 0)
        | (xys[:, :, :, 0] >= drivables.shape[2])
    )
    xys[outside] = 0
    # 1's mean drivable
...
def check_line_layer(drivables, dx, start, end, mapixes):
    ...
    line_interp = (
        start.view(B, 1, 2) * (1.0 - line_w) + end.view(B, 1, 2) * line_w
    )  # (B, L, 2)
...
def get_coll_point(drivables, dx, cars, lw, mapixes, return_iou=False):
    """
    ...
    B = cars.size(0)
    # sample grid aligned with agent coords in the world space
    xys_world = gen_car_coords(
        cars[:, :2], cars[:, 2:], 1, L, W, ls=lw[:, 0], ws=lw[:, 1]
    )[
        :, 0
    ]  # B x L x W x 2
    ...
    xys[outside] = 0
    # 1's mean drivable
    car_pix = drivables[front_ixes, xys[:, :, :, 1], xys[:, :, :, 0]].unsqueeze(
        -1
    )  # B x L x W x 1
    non_drivable = (
        car_pix == 0
    )  # torch.where(car_pix == 0, torch.ones_like(car_pix), torch.ones_like(car_pix)*np.nan)
    ...
    coll_pt = (
        coll_pt / num_non_drivable
    )  # results in nan for any vehicle fully on drivable
...
def viz_scene_graph(
    ...
    scene_graph = normalize_scene_graph(
    ...
    if center_viz:
        ...
        fut_traj = (
            future_pred[binds][:, :, :, :2]
            if future_pred is not None
            else scene_graph.future_gt[:, :, :2].view(NA, 1, FT, 2)[binds]
        )
        past_traj = (
            scene_graph.past_gt[:, :, :2]
            .view(NA, 1, PT, 2)
            .expand(NA, NS, PT, 2)[binds]
        )
        ...
        crop_pos = torch.cat(
            [viz_cent.view(1, 2), torch.Tensor([[1.0, 0.0]]).to(viz_cent)], dim=1
        )
    ...
    crop_objs, crop_lw = map_env.objs2crop(
        crop_pos[binds][0],
    ...
    if show_gt and future_pred is not None:
        ...
        gt_crop_objs, _ = map_env.objs2crop(
            crop_pos[binds][0],
    ...
    scene_graph = normalize_scene_graph(scene_graph, state_normalizer, att_normalizer)
...
def viz_map_crop_video(
    ...
    for s in range(NS):
        ...
        for t in range(NT):
            viz_map_crop(
                x,
                os.path.join(cur_outdir, "frame%04d.png" % (t)),
                car_kin=car_kin[:, s : s + 1, t : t + 1, :],
                car_lw=car_lw,
                viz_traj=viz_traj,
                indiv=indiv,
                car_colors=car_colors,
        ...
        create_video(
            os.path.join(cur_outdir, "frame%04d.png"), cur_outdir + ".mp4", fps
        )
...
def viz_map_crop(
    ...
    if car_kin is not None and car_lw is not None:
        render_obj_observation(
            ...
            car_lw,
            viz_traj=viz_traj,
            color_traj=color_traj,
            color_traj_bounds=color_traj_bounds,
            color=car_colors,
            alpha=car_alpha,
...
def render_obj_observation(
    ...
    if use_color_traj:
        ...
        color_traj_norm = mpl.colors.Normalize(
            vmin=color_traj_bounds[0], vmax=color_traj_bounds[1]
        )
    if car_kin is not None and car_lw is not None:
        if viz_traj:
            ...
            if len(car_kin.size()) == 3:
                ...
                for n in range(NA):
                    ...
                    cur_linewidth = (
                        traj_linewidth[n] if traj_linewidth is not None else 1.0
                    )
                    cur_msize = (
                        traj_markersize[n] if traj_markersize is not None else 8.0
                    )
                ...
                for n in range(NA):
                    for s in range(NS):
                        ...
                        if cur_traj.size(0) > 0:
                            if use_color_traj:
                                cur_col = color_traj_cmap(
                                    color_traj_norm(color_traj[n, s].cpu().item())
                                )
                            else:
                                cur_col = (
                                    color[n] if color is not None else plt_color(n % 9)
                                )
                            plt.plot(
                                cur_traj[:, 0],
                                cur_traj[:, 1],
            ...
            if viz_init_car:
                ...
                if len(car_kin.size()) == 3:
                    ...
                    for n in range(NA):
                        ...
                        cur_col = color[n] if color is not None else plt_color(n % 9)
                        cur_alph = alpha[n] if alpha is not None else 0.7
                        nonnan_idx = torch.nonzero(
                            ~torch.isnan(car_kin[n, :, 0]), as_tuple=True
                        )[0][0]
                        plot_car(
                            car_kin[n, nonnan_idx, 0],
                            car_kin[n, nonnan_idx, 1],
                            car_kin[n, nonnan_idx, 2],
                            car_kin[n, nonnan_idx, 3],
                            car_lw[n, 0],
                            car_lw[n, 1],
                            cur_col,
                            alpha=cur_alph,
                    ...
                    for n in range(NA):
                        for s in range(1):
                            ...
                            cur_alph = alpha[n] if alpha is not None else 0.7
                            nonnan_idx = torch.nonzero(
                                ~torch.isnan(car_kin[n, s, :, 0]), as_tuple=True
                            )[0][0]
                            plot_car(
                                car_kin[n, s, nonnan_idx, 0],
                                car_kin[n, s, nonnan_idx, 1],
                                car_kin[n, s, nonnan_idx, 2],
                                car_kin[n, s, nonnan_idx, 3],
                                car_lw[n, 0],
                                car_lw[n, 1],
                                cur_col,
                                alpha=cur_alph,
                    ...
                    for t in range(T):
                        if torch.sum(torch.isnan(car_kin[n, t])) == 0:
                            ...
                            cur_alpha = (
                                cur_alpha
                                if t == 0
                                else 0.1 + (1.0 - (float(t) / T)) * 0.2
                            )  # decrease visiblity over time
                            cur_col = (
                                color[n] if color is not None else plt_color(n % 9)
                            )
                            plot_car(
...
def plot_car(x, y, hx, hy, l, w, color="b", alpha=0.7, no_heading=False):
    plot_box(
        np.array([x.item(), y.item(), hx.item(), hy.item()]),
        [l.item(), w.item()],
        color=color,
        alpha=alpha,
